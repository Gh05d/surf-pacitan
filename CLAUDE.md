...
**Resolved mismatch (2026-05-29):** Previously `SpotMap.tsx`'s coordinates conflicted with the config labels. The coordinates were right (Teleng Ria westmost `111.0790`, the river mouth easternmost `~111.10`); the labels were wrong. SpotMap's easternmost river-mouth marker is now `Pancer` (was mislabeled `Pancer Door`) and the middle marker `Pancer Door`. **Still open:** the data-fetch point `LOCATION` (`111.13`) sits ~3 km east of the actual breaks (`111.079–111.103`) — acceptable for regional swell/wind, not yet nudged onto the bay.
