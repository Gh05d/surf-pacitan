import type { ForecastDay, HourlyData, SpotName } from "./types";
import { SPOT_DISPLAY } from "./spots";
...
/**
 * Best remaining window starting at or after `fromHour` — powers the
 * today-only "best window from now" banner. Reuses the full candidate
 * ranking on the remaining hours; returns rank 1 or null when nothing
 * non-red is left.
 */
export function bestRemainingWindow(forecast: ForecastDay, fromHour: number): CandidateWindow | null {
  const remaining = forecast.hourly.filter((h) => h.hour >= fromHour);
  if (!remaining.length) return null;
  const candidates = computeCandidateWindows({ ...forecast, hourly: remaining });
